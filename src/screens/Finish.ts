...
class Finish extends Drawable {
    ...
    constructor(text: string, parent?: Drawable){
        ...
        const background = new Rect({width: 720, height: 360}, '#aaa', 0, container);
        background.pivot.set(360, 180);
        background.alpha = 0.6;
        const border = new Rect({width: 720, height: 360}, 'black', 5, container);
        border.pivot.set(360, 180);

        const ttt = new TextDrawable(text, {family: 'arial', size: 50, color: 'black'}, container);
